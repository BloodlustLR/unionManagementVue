    ...
    <el-dialog v-model="addUnionModal" title="新增联盟" width="500px" :close-on-click-modal="false">
        <div>
            <div style="width:400px;margin:0 auto;text-align:center;height:60px;line-height:60px">
                联盟名:<el-input v-model="unionInfo.name" placeholder="请输入联盟名称" style="width:200px;margin-left:10px;"/>
            </div>
        </div>
        <template #footer>
...
export default {
    ...
    methods:{
        ...
        addUnion(){
            if(isEmpty(this.unionInfo.name)){
                ElMessage({
                    message: '请先补全数据',
                    type: 'warning',
                })
        ...
        configUnion(){
            if(isEmpty(this.unionInfo.unionId)||isEmpty(this.unionInfo.name)){
                ElMessage({
                    message: '请先补全数据',
                    type: 'warning',
                })
